...
class Aggregate:
    ...
    def createMainParallelTestFile(self, func_names):
        with open(self.MAIN_FILE_NAME, 'w+') as f:
            ...
            f.write('choosenTests = {\n')
            for func_name in func_names + self.FILES_NO_SEPARATE_TESTS:
                f.write("'" + func_name + "',\n")
            f.write('};\n')
            f.write('vector_y = pararrayfun(nproc - nproc*0.1, @runAutogeneratedTestsOctaveParallel, choosenTests);\n')
            ...
            f.write('function result = runAutogeneratedTestsOctaveParallel(choosenTest)\n')
            f.write("if strcmp(choosenTest, '" + func_names[0] + "')\n")
            f.write('test ' + func_names[0] + ';\n')
            f.write('result = 1;\n')
            func_names.pop(0)
            for func_name in func_names + self.FILES_NO_SEPARATE_TESTS:
                f.write("elseif strcmp(choosenTest, '" + func_name + "')\n")
